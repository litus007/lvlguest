# LVCLITS — Visor Web (Guest)

Pàgina web independent (sense Tauri, sense instal·lació d'escriptori)
que fa de "visor remot": algú introdueix el codi de sala i rep la
pantalla que un Host de l'app **LVCLITS** està compartint, amb opció
de pantalla completa. Comparteix la mateixa identitat visual (logotip,
colors, marcs de cantonades) que l'app d'escriptori.
...
**Important:** aquest visor és de només-lectura. No envia tecles ni
ratolí al Host — només rep vídeo. Qui vulgui compartir la seva pròpia
pantalla (rol Host) sempre necessitarà l'app d'escriptori, perquè
capturar pantalla i injectar inputs no es pot fer des d'una pàgina web.

## Per què és possible

El costat "Guest" de l'app d'escriptori ja és WebRTC 100% estàndard de
navegador (`RTCPeerConnection`, `RTCDataChannel`, WebCodecs) — mai crida
cap funció nativa de Tauri. Aquest projecte reprodueix exactament el
mateix protocol de senyalització (Supabase Realtime: presència +
broadcast d'ofertes/respostes/ICE) i el mateix mecanisme de recepció de
vídeo (frames H.264 crus per un `RTCDataChannel`, decodificats amb la
WebCodecs API sobre un `<canvas>`), així que és compatible amb qualsevol
Host ja desplegat, sense tocar-hi res.
